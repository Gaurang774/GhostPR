...
// ─── Tool Implementation ─────────────────────────────────────────────────────
export async function getFileDecisions(input, db, hindsight) {
    const { file, module, intent } = input;
    const normalizedFile = normalizeFilePath(file);
    // 1. Query SQLite for decisions matching file + module
    const rows = db.exec(`SELECT id, file_path, module, summary, reason, result, lesson,
    ...
    // 2. Rule 1 — No memory found → return clean null message
    if (decisions.length === 0) {
        // Optionally check Hindsight for semantic matches
        if (hindsight) {
            try {
                const hindsightResult = await hindsight.recall(`${normalizedFile} ${module} ${intent}`, { tags: [module, 'decision'], limit: 3 });
    ...
    const warningCards = decisions.map((d) => ({
        decisionId: d.id,
...
// ─── Formatting ───────────────────────────────────────────────────────────────
function formatWarningCards(cards, file, module, intent) {
    const lines = [];
    ...
    for (const card of cards) {
        if (card.isHighConfidence) {
            ...
            lines.push(`⚠ WARNING — Historical Decision (confidence: ${(card.confidence * 100).toFixed(0)}%)`);
...
function formatHindsightFallback(memories, file, module) {
    const lines = [];
...
//# sourceMappingURL=getFileDecisions.js.map