...
export function markDeprecated(
  ...
  const existing = db.exec(
    `SELECT id, summary, file_path, status FROM decisions WHERE id = '${escapeSQL(decisionId)}' LIMIT 1`
  );
  ...
  const [id, summary, filePath, currentStatus] = existing[0]!.values[0]! as [string, string, string, string];
