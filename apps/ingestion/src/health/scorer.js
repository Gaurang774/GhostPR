...
function statusFromConfidence(confidence) {
    if (confidence >= THRESHOLD_QUESTIONABLE)
        return 'active';
    if (confidence >= THRESHOLD_DEPRECATED)
        return 'questionable';
    return 'deprecated';
}
// ─── Public API ───────────────────────────────────────────────────────────────
 ...
 */
export function applyTimeDecay(decision, now = new Date()) {
 ...
 */
export function shouldShowWarning(confidence) {
